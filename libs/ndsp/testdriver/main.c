...
int main(int argc, char** argv)
{
   ...
   *----------------------------------------------------------------------------*/
  {
    ...
    if(testFlags == eTESTFLAG_HELP)
    {
    	printf(" \n End of -help .. all other options ignored \n ");
    	return 0;
    }

    testType=PackageGetTestType();
    // cache warming support for profiler
    noWarmup=(testFlags & eTESTFLAG_NOWARMUP)?1:0;
    #if defined (COMPILER_XTENSA) && (MEM_MODEL!=2)
    if (noWarmup)
    {
        fprintf(stderr,"-nowarmup option is ignored. It should be used if code is built with MEM_MODEL=2\n");
    }
    #endif
