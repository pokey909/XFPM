...
void te_processFxn_mmlt( tTestEngContext * context )
{
  ...
  typedef void tFxn_noScr(            void * z,   const void * x, const void * y, int M, int N, int P        );
  typedef void tFxnlshscr(void *pScr, void * z,   const void * x, const void * y, int M, int N, int P, int lsh);
  typedef void tFxnlshnoscr(          void * z,   const void * x, const void * y, int M, int N, int P, int lsh);
  void *X, *Y, *Z;
  int M, N, P, lsh;
  int noScratch;
  size_t scrSz;
  ...
  scrSz=0;
  noScratch=context->desc->extraPtr==NULL;
  if(!noScratch) 
  {
  ...
  {
  case MTX_PLAIN:     
      if (noScratch) ((tFxn_noScr*   )context->target.fut)(Z, X, Y, M, N, P      );
      else           ((tFxn*   )context->target.fut) (pScr,Z, X, Y, M, N, P      );
     if (pScr) freeAlign(pScr);
     break;
  case MTX_PLAIN_LSH:
    {   
        if (noScratch) ((tFxnlshnoscr*)context->target.fut)    (Z, X, Y, M, N, P, lsh ); 
        else    ((tFxnlshscr*)context->target.fut)(pScr, Z, X, Y, M, N, P, lsh ); 
    }
