...
void te_processFxn_matinv( tTestEngContext * context )
{
    ...
    typedef void tFxnFloatnoScr (const void * X);
    typedef int  tFxnFixed      (void * pScr, const void * X,int qA);
    typedef int  tFxnFixednoScr (const void * X,int qA);
    const tMtxInvApi* pApi=(const tMtxInvApi*)context->desc->extraPtr;
    void *X, *Z;
    void *pScr;
    int noScratch=pApi->getScratchSize==NULL;
    ...
    pScr = mallocAlign(noScratch ? 0:pApi->getScratchSize(),0);
    if (pScr==NULL )  
    {
    ...
    case MTXINV_PLAIN:    
        if (isFloatingPoint)
        {
            if (noScratch)   ((tFxnFloatnoScr*)context->target.fut)(  Z );
            else             ((tFxnFloat*)context->target.fut)( pScr, Z ); 
        }
        else
        {
            ...
            if (noScratch)  qB=((tFxnFixednoScr*)context->target.fut)(  x,qA ); 
            else            qB=((tFxnFixed*)context->target.fut)( pScr, x,qA ); 
