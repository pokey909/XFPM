...
int te_load_fft( tTestEngContext * context )
{
  ...
  context_fft->ext.fnGetFFT_handle=((const tTestEngDesc_fft *)context->desc)->fnGetFFT_handle;
                         ...
                         tVec * xVec, tVec * yVec)
{
  ...
  else
  {
      ...
      {
          ASSERT(context_fft->fnGetFFT_handle!=NULL);
          const void *h = context_fft->fnGetFFT_handle(N, MAKE_FFT_ID(context->args.caseType, context->desc->extraParam, context->desc->fmt));
          if (h==NULL)
          {
