...
#define MEM_ALIGNER  __declspec(align(16))
#else
#define MEM_ALIGNER __attribute__ ((aligned(16)))
#endif

#if defined (COMPILER_XTENSA) && defined(MEM_MODEL)
  //#if MEM_MODEL==1
  //  #if (XCHAL_DATARAM0_SIZE <= (1*1024*1024))
  ...
  //#endif
	/* for MEM_MODEL=2 place to sram, otherwise place to dram0/dram0  */
		...
		#error unsupported MEM_MODEL
          ...
          #elif ((MEM_MODEL==1)&&(XCHAL_NUM_DATARAM>1))
          #define DRAM1 ".dram1.data"
          #elif((MEM_MODEL==1)&&(XCHAL_NUM_DATARAM==1))
    ...
    #else
    	#define PLACE_IN_DRAM1 __attribute__ ((section(DRAM0)))
    #endif
...
int main_mips( int phaseNum, int isFull, int isVerbose, int breakOnError )
{
    ...
    fprintf(stderr,"CCOUNT register not present in the configuration or XT_RSR_CCOUNT not defined! Cycle measurements might be inaccurate!\n");
#endif
#if !defined(MEM_MODEL)
    fprintf(stderr,"MEM_MODEL is not defined! Cycle measurements might be inaccurate!\n");
#endif
#endif
