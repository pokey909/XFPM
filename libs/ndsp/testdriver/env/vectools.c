  ...
  case FMT_FLOAT64:
    {
      ...
      for ( n=0; n<N; n++ ) z[n] = ldexp( (double)x[n], -15 );
      break;
    }
  default:
    ASSERT(0);
...
void bvecFromPcm16( tVec * vec, const fract16 * x, int N, int L, int bexp, int16_t shift[] )
{
  ...
  case FMT_FLOAT64:
    {
      float64_t * z = (float64_t *)vecGetElem( vec, 0 );
      for ( n=0; n<N*L; n++ ) z[n] = ldexp( (double)x[n], -15 );
      break;
    }
  default:
    ASSERT(0);
                     ...
                     int N, int L, int bexp, int16_t shift[])
{
    ...
    case FMT_FLOAT64:
    {
                        ...
                        { 
                            for (n = 0; n < N; n++)
                            {
                                z[2 * (n*L + l)    ] = ldexp((double)x[2 * n    ], -15);
                                z[2 * (n*L + l) + 1] = ldexp((double)x[2 * n + 1], -15);
                            }
                        }
                        break;
    }
  ...
  case FMT_FRACT16:
    {
      ...
      for ( n=0; n<N; n++ ) z[n] = ldexp( (double)x[n], -shift-15 );
      break;
    }
  case FMT_FRACT32:
    {
      fract32 * x =(fract32 *) vecGetElem( vec, 0 );
      for ( n=0; n<N; n++ ) z[n] = ldexp( (double)x[n], -shift-31 );
      break;
    }
  case FMT_FLOAT32:
    {
                ...
                int L, int16_t shift[])
{
    ...
    case FMT_FRACT16:
    {
                        ...
                        for (l = 0; l<L; l++, z+=2*N, shift++)
                        {
                            for (n = 0; n < N; n++)
                            {
                                z[2*n    ] = ldexp((double)x[2 * (n*L + l) + 0 ], -(*shift) - 15);
                                z[2*n + 1] = ldexp((double)x[2 * (n*L + l) + 1 ], -(*shift) - 15);
                            }
                        }
                        break;
    }
    case FMT_FRACT32:
    {
                        ...
                        for (l = 0; l<L; l++, z += 2 * N, shift++)
                        {
                            for (n = 0; n < N; n++)
                            {
                                z[2 * n    ] = ldexp((double)x[2 * (n*L + l) + 0], -(*shift) - 31);
                                z[2 * n + 1] = ldexp((double)x[2 * (n*L + l) + 1], -(*shift) - 31);
                            }
                        }                        
                        break;
    }
