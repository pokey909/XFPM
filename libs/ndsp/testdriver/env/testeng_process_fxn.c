...
/* Apply the target function to the test case data set:
 * vector Z (out), vector X (in), scalar Y (in) */
void te_processFxn_vZcvXcsYr( tTestEngContext * context )
{
  typedef void tFxn_fr16c ( complex_fract16 * z, const complex_fract16 * x, fract16 y, int N );
  typedef void tFxn_fr32c ( complex_fract32 * z, const complex_fract32 * x, fract32 y, int N );
  typedef void tFxn_fl32c ( complex_float   * z, const complex_float   * x, float32_t  y, int N );
  typedef void tFxn_fl64c ( complex_double  * z, const complex_double  * x, float64_t  y, int N );
  ...
  */
  ...
  switch ( context->desc->fmt )
  {
  case FMT_CPLX|FMT_FRACT16: ( (tFxn_fr16c*)fxn )((complex_fract16 *)Z, (const complex_fract16*)X, *(fract16*)Y,  N ); 	break;
  case FMT_CPLX|FMT_FRACT32: ( (tFxn_fr32c*)fxn )((complex_fract32 *)Z, (const complex_fract32*)X, *(fract32*)Y,  N );	break;
  case FMT_CPLX|FMT_FLOAT32: ( (tFxn_fl32c*)fxn )((complex_float   *)Z, (const complex_float  *)X, *(float32_t  *)Y,  N ); 		break;
  case FMT_CPLX|FMT_FLOAT64: ( (tFxn_fl64c*)fxn )((complex_double  *)Z, (const complex_double *)X, *(float64_t *)Y,  N ); 	break;
  default: ASSERT( 0 );
  }

} /* te_processFxn_vZcvXcsYr() */
