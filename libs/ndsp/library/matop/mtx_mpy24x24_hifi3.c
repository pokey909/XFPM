...

{
    ...
    const ae_f24x2 * restrict py0;
    const ae_f24x2 * restrict py1;
          ae_f24x2 * restrict pz;
          ae_f24x2 * restrict pz0;
          ae_f24x2 * restrict pz1;
             f24   * restrict scr = (f24 *)pScr;
    ...
    for (p = 0; p < (P >> 1); p++)
    {
        py0 = (const ae_f24x2 *)y;
        py1 = (const ae_f24x2 *)XT_ADDX4(P, (uintptr_t)py0);
        pz = (ae_f24x2 *)pScr;
        for (n = 0; n < (N >> 1); n++)
        {
            ay0 = AE_LA64_PP(py0);
            ay1 = AE_LA64_PP(py1);
            AE_LA32X2F24_IP(vt0, ay0, py0);
            AE_LA32X2F24_IP(vt1, ay1, py1);
            py0 = (const ae_f24x2 *)XT_ADDX8(P - 1, (uintptr_t)py0);
            py1 = (const ae_f24x2 *)XT_ADDX8(P - 1, (uintptr_t)py1);
            
            vy0 = AE_SEL24_HH(vt0, vt1);
            vy1 = AE_SEL24_LL(vt0, vt1);
            AE_S32X2F24_IP(vy0, pz, sizeof(ae_f24x2));
            AE_S32X2F24_IP(vy1, pz, sizeof(ae_f24x2));
        }
        if (N & 1)
        {
            ay0 = AE_LA64_PP(py0);
            AE_LA32X2F24_IP(vt0, ay0, py0);
            vy0 = AE_SEL24_HH(vt0, AE_ZERO24());
            vy1 = AE_SEL24_LL(vt0, AE_ZERO24());
            AE_S32X2F24_IP(vy0, pz, sizeof(ae_f24x2));
            AE_S32X2F24_IP(vy1, pz, sizeof(ae_f24x2));
        }
        ...
        pz = (ae_f24x2 *)z;
        pz0 = (ae_f24x2 *)pz;
        pz1 = (ae_f24x2 *)XT_ADDX4(P, (uintptr_t)pz0);
        px = (const ae_f24x2 *)(x);

        for (m = 0; m < (M >> 1); m++)
        {
            px0 = (const ae_f24x2 *)px;
            px1 = (const ae_f24x2 *)XT_ADDX4(N, (uintptr_t)px);
            ax0 = AE_LA64_PP(px0);
            ax1 = AE_LA64_PP(px1);
            py = (const ae_f24x2 *)pScr;
            ...
            {
                /* load data from 'x' */
                AE_LA32X2F24_IP(vx0, ax0, px0);
                AE_LA32X2F24_IP(vx1, ax1, px1);
                /* load data from 'y' */
                AE_L32X2F24_IP(vy0, py, sizeof(ae_f24x2));
                AE_L32X2F24_IP(vy1, py, sizeof(ae_f24x2));
            ...
            az0 = AE_ZALIGN64();
            az1 = AE_ZALIGN64();
            AE_SA32X2F24_IP(vz0, az0, pz0);
            AE_SA32X2F24_IP(vz1, az1, pz1);
            AE_SA64POS_FP(az0, pz0);
            AE_SA64POS_FP(az1, pz1);
            pz0 = (ae_f24x2 *)XT_ADDX8(P - 1, (uintptr_t)pz0);
            pz1 = (ae_f24x2 *)XT_ADDX8(P - 1, (uintptr_t)pz1);

            px = (const ae_f24x2 *)XT_ADDX8(N, (uintptr_t)px);
        }
        if (M & 1)
        {
            px0 = (const ae_f24x2 *)px;
    ...
    if (P & 1)
    {
        ...
        for (m = 0; m < (M >> 1); m++)
        {
            ...
            ACC00 = AE_SLAA64(ACC00, lsh);
            ACC10 = AE_SLAA64(ACC10, lsh);
            vz0 = AE_ROUND24X2F48SASYM(ACC00, ACC00);
            vz1 = AE_ROUND24X2F48SASYM(ACC10, ACC10);
            /* save values */
            AE_S32F24_L_I(vz0, (ae_f24 *)pz0, 0);
            AE_S32F24_L_I(vz1, (ae_f24 *)pz1, 0);
            pz0 = (ae_f24x2 *)XT_ADDX8(P, (uintptr_t)pz0);
            pz1 = (ae_f24x2 *)XT_ADDX8(P, (uintptr_t)pz1);
