...
DISCARD_FUN(void,bqriirf_df2,( bqriirf_df1_handle_t restrict _bqriir,
                      float32_t *       z,
...
#if HAVE_VFPU
{
    ...
    if (M & 1)
    {
        ...
        dy0 = XT_xtfloat_loadi((xtfloat *)pDrd,1*sizeof(float32_t)); 
        dy1 = XT_xtfloat_loadi((xtfloat *)pDrd,2*sizeof(float32_t)); 
        ...
        for (n = 0; n < N; n++)
        {
            XT_xtfloat_loadip(dx0,castxcc (const xtfloat,pX),sizeof(x[0]));
            dy2=dy1;
            dy1=dy0;
            dy0=dx0;
        ...
        XT_xtfloat_storei(dy0,(xtfloat *)pDwr,1*sizeof(float32_t));
        XT_xtfloat_storei(dy1,(xtfloat *)pDwr,2*sizeof(float32_t));
        pX=(const ae_int32x2*)z;/* switch pointer to the input data to the pointer to the output */
    }
    /* final scaling */
    {
...
#else // scalar FPU: usess different layout
{
#define VLEN 4  /* vector length */
    bqriirf_df2_t *state;
    const float32_t * restrict cf;
    ...
    while (P > 0)/* vector length is divided by 2 in each iteration */
    {
        ...
        for (m=M0; m<M1; m+=P)
        {
            float32_t dx0[VLEN],dy0[VLEN],dy1[VLEN],dy2[VLEN];
            ...
            for (n=0;n<N;n++)
            {
                ...
                {
                    float32_t tmp;
                    tmp    =dx0[p]-a2[p]*dy2[p]-a1[p]*dy1[p];
                    dx0[p] =b2[p]*dy2[p]+b1[p]*dy1[p]+b0[p]*tmp;
                    dy0[p] =tmp;
                }
                z[n]=dx0[P-1];
            }
