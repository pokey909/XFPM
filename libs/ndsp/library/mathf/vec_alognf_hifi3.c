...
#if HAVE_VFPU
{
  ...
  */
  ...
  for ( n=0; n<(N>>1); n++ )
  {
    ...
    AE_L32_IP( tb0, TBL, +1*4 );
    AE_L32_IP( tb1, TBL, +1*4 );
    AE_L32_IP( tb2, TBL, +1*4 );
    AE_L32_IP( tb3, TBL, +1*4 );
    AE_L32_IP( tb4, TBL, +1*4 );
    AE_L32_IP( tb5, TBL, +1*4 );
    AE_L32_XP( tb6, TBL, -6*4 ); // _XP instead of _IP results in a better schedule!
