...
int64_t vec_dot64x64i_fast (const int64_t * restrict x,const int64_t * restrict y,int N)
{
    ...
    for (n=0; n<N; n++)
    {
        AE_L64_IP(xw0, px, sizeof(int64_t));x0 = AE_MOVINT32X2_FROMINT64(xw0);
        AE_L64_IP(yw0, py, sizeof(int64_t));y0 = AE_MOVINT32X2_FROMINT64(yw0);
        AE_MULA32U_LL(ACC,x0,y0);

#ifdef AE_MULAAD32_HL_LH
        AE_MULAAD32_HL_LH(W0,x0,y0);
#else
        AE_MULA32_LH(W0,x0,y0);
        x0 = AE_SEL32_LH(x0, x0);
        AE_MULA32_LL(W0,x0,y0);
#endif
    }
    W0 = AE_SLAI64(W0,32);
    ACC= AE_ADD64(ACC,W0);
