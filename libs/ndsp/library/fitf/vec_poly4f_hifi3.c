...
{
    int n;
    xtfloatx2 ALIGN(16) temp[5];
    xtfloatx2 vxf;
    xtfloatx2 t, vc0f, vc1f, vc2f, vc3f, vc4f,s;
    xtfloat c0f, c1f, c2f, c3f, c4f;
    xtfloat zf;
    ...
    const xtfloat *  restrict pc = (const xtfloat *)c;
        xtfloatx2* restrict pz = (xtfloatx2 *)z;
    const xtfloatx2* restrict pC = (const xtfloatx2 *)temp;
    ae_valign x_align, z_align;
    NASSERT(x);
    NASSERT(c);
    NASSERT(z);
    ...
    XT_LSIP(c0f, pc, sizeof(*pc)); temp[0]=vc0f = (c0f);
    XT_LSIP(c1f, pc, sizeof(*pc)); temp[1]=vc1f = (c1f);
    XT_LSIP(c2f, pc, sizeof(*pc)); temp[2]=vc2f = (c2f);
    XT_LSIP(c3f, pc, sizeof(*pc)); temp[3]=vc3f = (c3f);
    XT_LSIP(c4f, pc, sizeof(*pc)); temp[4]=vc4f = (c4f);

  __Pragma("no_reorder")
    for (n = 0; n < (N>>1); n ++)
    {
        XT_LASX2IP(vxf, x_align, px);
       /*vc3f=XT_LSX2I(pC,3*sizeof(xtfloatx2)); */t=vc3f;XT_MADD_SX2(t, vxf, vc4f);s=t;
       /*vc2f=XT_LSX2I(pC,2*sizeof(xtfloatx2)); */t=vc2f;XT_MADD_SX2(t, vxf, s   );s=t;
        vc1f=XT_LSX2I(pC,1*sizeof(xtfloatx2));    t=vc1f;XT_MADD_SX2(t, vxf, s   );s=t;
        vc0f=XT_LSX2X(pC,0*sizeof(xtfloatx2));    t=vc0f;XT_MADD_SX2(t, vxf, s   );s=t;
        XT_SASX2IP(s, z_align, pz);
    }
    AE_SA64POS_FP(z_align, pz);
    if (N & 1)
    {
        vxf=XT_LSI((const xtfloat*)px, 0);
      /*vc3f=XT_LSX2I(pC,3*sizeof(xtfloatx2));  */t=vc3f;XT_MADD_SX2(t, vxf, vc4f);s=t;
      /*vc2f=XT_LSX2I(pC,2*sizeof(xtfloatx2));  */t=vc2f;XT_MADD_SX2(t, vxf, s   );s=t;
        vc1f=XT_LSX2I(pC,1*sizeof(xtfloatx2));    t=vc1f;XT_MADD_SX2(t, vxf, s   );s=t;
        vc0f=XT_LSX2X(pC,0*sizeof(xtfloatx2));    t=vc0f;XT_MADD_SX2(t, vxf, s   );s=t;
