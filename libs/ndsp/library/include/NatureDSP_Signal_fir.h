...
bkfir16x16_handle_t   bkfir16x16_init  (void * objmem, int M, int extIR, const int16_t  * restrict h);
bkfir24x24_handle_t   bkfir24x24_init  (void * objmem, int M, int extIR, const f24      * restrict h);
...
/* Returns: handle to the object */
cxfir16x16_handle_t   cxfir16x16_init  (void * objmem, int M, int extIR, const complex_fract16 * restrict h);
cxfir24x24_handle_t   cxfir24x24_init  (void * objmem, int M, int extIR, const complex_fract32 * restrict h);
...
#define FIR_CONVOLA32X32_SCRATCH_SIZE( N, M )    ( 4*( 2*(M) + (N) + 7 )                  )
#define FIR_CONVOLAF_SCRATCH_SIZE( N, M )        ( 4*( 2*(M) + (N) + 7 )                  )
...
/*-------------------------------------------------------------------------
  ...
  N >= M-1    minimum allowed length of vector x is the length of y minus one
-------------------------------------------------------------------------*/
#define FIR_LCONVOLA16X16_SCRATCH_SIZE( N, M )    ( 2*(N + M + 10 + 8) )
#define FIR_LCONVOLA32X32_SCRATCH_SIZE( N, M )    ( 4*(N + M + 10) )
...
#define FIR_XCORRA16X16_SCRATCH_SIZE( N, M )      ( 2*( 2*(M) + (N) + 7 ) + 6           )
#define FIR_XCORRA24X24_SCRATCH_SIZE( N, M )      ( 4*( 2*(M) + (N) + 7 )               )
#define FIR_XCORRAF_SCRATCH_SIZE( N, M )          ( 4*((M)+(N)-1+2)                           )
#define FIR_XCORRA32X32_SCRATCH_SIZE( N, M )      ( 4*( 2*(M) + (N) + 7 )               )
#define CXFIR_XCORRAF_SCRATCH_SIZE( N, M )        ( 0                                   )
#define FIR_XCORRA32X16_SCRATCH_SIZE( N, M )      ( 4*( (M) + (N) + 2 ) + 2*( (M) + 6 ) )
#define CXFIR_XCORRA32X32_SCRATCH_SIZE( N, M )          ( 0 ) // just for compatibility with other HiFi APIs
...
#define FIR_LXCORRA32X32_SCRATCH_SIZE(N, M)      ( 4*(N + M + 10) )
...
#define FIR_ACORRAF_SCRATCH_SIZE( N )           ( 4*( 2*(N) - 1+2) )
