...
void vec_poly8_24x24 (f24 * restrict z, const f24 * restrict x, const f24 * restrict c, int lsh, int N )
{
    ...
    AE_LA32X2F24_IP(tt, c_align, castxcc(const ae_f24x2,pc)); s=AE_SLAI32(tt, 8); AE_S32X2_I(s,temp,3*sizeof(ae_int32x2));
    AE_LA32X2F24_IP(tt, c_align, castxcc(const ae_f24x2,pc)); s=AE_SLAI32(tt, 8); AE_S32X2_I(s,temp,4*sizeof(ae_int32x2));
    pc=(const ae_int32  *)temp;
    __Pragma("no_reorder");
  
    x_align = AE_LA64_PP(px);
    z_align = AE_ZALIGN64();
    c7f=AE_L32_I (pc, 7*sizeof(*pc));
    c6f=AE_L32_I (pc, 6*sizeof(*pc));
    c5f=AE_L32_I (pc, 5*sizeof(*pc));
    c8f=AE_L32_X (pc, 8*sizeof(*pc));
    WUR_AE_SAR(lsh);
    for (n=0; n<(N&~1); n+=2)
    { 
        AE_LA32X2F24_IP(tt, x_align, px); vxf=tt; vxf=AE_SLAI32(vxf, 8);
                                          t=c7f; AE_MULAFP32X2RAS(t, vxf, c8f); s=(t);
                                          t=c6f; AE_MULAFP32X2RAS(t, vxf, s  ); s=(t);
                                          t=c5f; AE_MULAFP32X2RAS(t, vxf, s  ); s=(t);
