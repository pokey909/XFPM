...
static void dct3p(uint64_t *x,uint64_t* y,int N,const tdct4_twd_fr16* pdct4_twd)
{
    ...
    for (k=1; k<(N>>2); k++)
    {
        ...
        b0re = AE_SUBADD32(a0re, a1re);
        b0im = AE_SUBADD32(a0im, a1im);
        b1re = AE_ADDSUB32(a0re, a1re);
        b1im = AE_ADDSUB32(a0im, a1im);
        Y0 = AE_SEL32_LH(b1re, b1re);
        Y1 = AE_SEL32_LH(b1im, b1im);

        a0re = AE_MULFC32X16RAS_L(Y0, cs);
        a0im = AE_MULFC32X16RAS_L(Y1, cs);
    ...
    {
        X0 = ae_f32x2_loadi(py0, 0);
        X1 = ae_f32x2_loadi(py1, 0);
    ...
    {
        ae_f32x2_loadip (X0, px0, sizeof(ae_f32x2));
        ae_f32x2_loadip (X1, px0, sizeof(ae_f32x2));
        Y0 = AE_SEL32_LL(X0, X1);
        Y1 = AE_SEL32_HH(X0, X1);
        ae_f32x2_storeip(Y0, py0, 2*sizeof(ae_f32x2));
        ae_f32x2_storeip(Y1, py0, 2*sizeof(ae_f32x2));

        ae_f32x2_loadxp (X0, px1, -(int)sizeof(ae_f32x2));
        ae_f32x2_loadxp (X1, px1, -(int)sizeof(ae_f32x2));
        Y0 = AE_SEL32_HH(X1, X0);
        Y1 = AE_SEL32_LL(X1, X0);
        ae_f32x2_storeip(Y0, py1, 2*sizeof(ae_f32x2));
        ae_f32x2_storeip(Y1, py1, 2*sizeof(ae_f32x2));
    }
}
...
static void ifft_cplx32x16_pair (uint64_t* y,uint64_t* x, const complex_fract16* twd,  int N)
{
    ...
    for ( n=0; n<stride; n++ )
    {
        ...
        ae_f32x2_loadip(a01, px0, sizeof(ae_f32x2));
        ae_f32x2_loadip(a00, px0, sizeof(ae_f32x2));
        ae_f32x2_loadip(a11, px1, sizeof(ae_f32x2));
        ae_f32x2_loadip(a10, px1, sizeof(ae_f32x2));
        ae_f32x2_loadip(a21, px2, sizeof(ae_f32x2));
        ae_f32x2_loadip(a20, px2, sizeof(ae_f32x2));
        ae_f32x2_loadip(a31, px3, sizeof(ae_f32x2));
        ae_f32x2_loadip(a30, px3, sizeof(ae_f32x2));
        a00 = AE_F32X2_SRAI(a00, 2);
        a01 = AE_F32X2_SRAI(a01, 2);
        a10 = AE_F32X2_SRAI(a10, 2);
        a11 = AE_F32X2_SRAI(a11, 2);
        a20 = AE_F32X2_SRAI(a20, 2);
        a21 = AE_F32X2_SRAI(a21, 2);
        a30 = AE_F32X2_SRAI(a30, 2);
        a31 = AE_F32X2_SRAI(a31, 2);

        b00 = AE_ADD32(a00, a20);
        b01 = AE_ADD32(a01, a21);
        b10 = AE_ADD32(a10, a30);
        b11 = AE_ADD32(a11, a31);
        b20 = AE_SUB32(a00, a20);
        b21 = AE_SUB32(a01, a21);
        b30 = AE_SUB32(a10, a30);
        b31 = AE_SUB32(a11, a31);

        b30 = AE_SEL32_LH(b30, b30);
        b31 = AE_SEL32_LH(b31, b31);
                                                              
        a00 = AE_ADD32(b00, b10);
        a01 = AE_ADD32(b01, b11);
        a10 = AE_ADDSUB32(b20, b30);
        a11 = AE_ADDSUB32(b21, b31);
        a20 = AE_SUB32(b00, b10);
        a21 = AE_SUB32(b01, b11);
        a30 = AE_SUBADD32(b20, b30);
        a31 = AE_SUBADD32(b21, b31);
        ...
        ae_f32x2_storeip(b00, py0, sizeof(ae_f32x2));
        ae_f32x2_storeip(b01, py0, sizeof(ae_f32x2));
        ae_f32x2_storeip(b20, py1, sizeof(ae_f32x2));
        ae_f32x2_storeip(b21, py1, sizeof(ae_f32x2));
        ae_f32x2_storeip(b10, py2, sizeof(ae_f32x2));
        ae_f32x2_storeip(b11, py2, sizeof(ae_f32x2));
        ae_f32x2_storeip(b30, py3, sizeof(ae_f32x2));
        ae_f32x2_storeip(b31, py3, sizeof(ae_f32x2));
    }
    ...
    if ( stride == 1 )
    {
        ...
        {
            ae_f32x2_loadip(a00, px0,   sizeof(ae_f32x2));
            ae_f32x2_loadip(a01, px0, 7*sizeof(ae_f32x2));
            ae_f32x2_loadip(a10, px1,   sizeof(ae_f32x2));
            ae_f32x2_loadip(a11, px1, 7*sizeof(ae_f32x2));
            ae_f32x2_loadip(a20, px2,   sizeof(ae_f32x2));
            ae_f32x2_loadip(a21, px2, 7*sizeof(ae_f32x2));
            ae_f32x2_loadip(a30, px3,   sizeof(ae_f32x2));
            ae_f32x2_loadip(a31, px3, 7*sizeof(ae_f32x2));
            a00 = AE_F32X2_SRAI(a00, 2);
            a01 = AE_F32X2_SRAI(a01, 2);
            a10 = AE_F32X2_SRAI(a10, 2);
            a11 = AE_F32X2_SRAI(a11, 2);
             ...
             * conj(x) == -j*swap(x) =>
             * ifft(x) == conj(fft(conj(x)) == swap(fft(swap(x))) */
            ae_f32x2_storex(a01, py0, idx+0*sizeof(ae_f32x2));
            ae_f32x2_storex(a00, py0, idx+1*sizeof(ae_f32x2));
            ae_f32x2_storex(a11, py1, idx+0*sizeof(ae_f32x2));
            ae_f32x2_storex(a10, py1, idx+1*sizeof(ae_f32x2));
            ae_f32x2_storex(a21, py2, idx+0*sizeof(ae_f32x2));
            ae_f32x2_storex(a20, py2, idx+1*sizeof(ae_f32x2));
            ae_f32x2_storex(a31, py3, idx+0*sizeof(ae_f32x2));
            ae_f32x2_storex(a30, py3, idx+1*sizeof(ae_f32x2));
    ...
    else
    {
        ...
        for ( n=0; n<(N>>1); n++ )
        {
            ae_f32x2_loadip(a00, px0, 4*sizeof(ae_f32x2));
            ae_f32x2_loadip(a01, px1, 4*sizeof(ae_f32x2));
            ae_f32x2_loadip(a10, px2, 4*sizeof(ae_f32x2));
            ae_f32x2_loadip(a11, px3, 4*sizeof(ae_f32x2));
            a00 = AE_F32X2_SRAI(a00, 1);
            a01 = AE_F32X2_SRAI(a01, 1);
            a10 = AE_F32X2_SRAI(a10, 1);
            a11 = AE_F32X2_SRAI(a11, 1);

            b00 = AE_ADD32(a00, a10);
            b01 = AE_ADD32(a01, a11);
            b10 = AE_SUB32(a00, a10);
            b11 = AE_SUB32(a01, a11);

            /* Real and imaginary parts are swapped on the first and last stages to
             * inverse the FFT:
             * conj(x) == -j*swap(x) =>
             * ifft(x) == conj(fft(conj(x)) == swap(fft(swap(x))) */
            ae_f32x2_storex(b01, py0, idx);
            ae_f32x2_storex(b00, py1, idx);
            ae_f32x2_storex(b11, py2, idx);
            ae_f32x2_storex(b10, py3, idx);
...
static void dct3p_N16(uint64_t *x,uint64_t* y,const tdct4_twd_fr16* pdct4_twd)
{
    ...
    px0 = (ae_f32x2 *)(x);
    px1 = (ae_f32x2 *)(x+N-1);
    px2 = (ae_f32x2 *)(x+N/2-1);
    px3 = (ae_f32x2 *)(x+N/2+1);
    py0 = (ae_f32x2 *)(y);
    py1 = (ae_f32x2 *)(y+N-1);
...

    {
        ...
        a0re = AE_F32X2_SRAI(X0, 1);
        a1re = AE_F32X2_SRAI(X1, 1);
    ...
    for (k=1; k<N/4; k++)
    {
        ...
        ae_f32x2_loadxp(X0, px2, -(int)sizeof(ae_f32x2));
        ae_f32x2_loadip(X1, px3,       sizeof(ae_f32x2));
        ...
        ae_f32x2_storeip(a0re,py0,      sizeof(ae_f32x2));
        ae_f32x2_storeip(a0im,py0,      sizeof(ae_f32x2));
        ae_f32x2_storexp(a1im,py1,-(int)sizeof(ae_f32x2));
        ae_f32x2_storexp(a1re,py1,-(int)sizeof(ae_f32x2));
    }
    {
        AE_L32_IP(t32, ptwd1, -2*(int)sizeof(int16_t)); cs = AE_MOVINT16X4_FROMINT32X2(t32);
        cs = AE_SHORTSWAP(cs);
        ...
        ae_f32x2_storexp(Y1,py1,-(int)sizeof(ae_f32x2));
        ae_f32x2_storexp(Y0,py1,-(int)sizeof(ae_f32x2));
    }
    __Pragma("no_reorder");
    /* IFFT (on pairs of data, N=8) and final permutation */
    {
        ...
        px0 = (ae_f32x2 *)(y+0*2*stride);
        px1 = (ae_f32x2 *)(y+1*2*stride);
        px2 = (ae_f32x2 *)(y+2*2*stride);
        px3 = (ae_f32x2 *)(y+3*2*stride);
        ...
        -------------------------------------------------------------------*/
        {
             ...
             * ifft(x) == conj(fft(conj(x)) == swap(fft(swap(x))) */
            {
                ...
                ae_f32x2_storex(a01, py0, 0*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a00, py0, 1*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a11, py1, 7*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a10, py1, 6*2*sizeof(ae_f32x2));
            }
            {
                ae_f32x2_loadip(a00, px0, 4*sizeof(ae_f32x2));
                ae_f32x2_loadip(a01, px1, 4*sizeof(ae_f32x2));
                ...
                a10 = AE_SEL32_HH(b11, b10);
                a11 = AE_SEL32_LL(b11, b10);
                ae_f32x2_storex(a01, py0, 4*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a00, py0, 5*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a11, py1, 3*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a10, py1, 2*2*sizeof(ae_f32x2));
            }
            {
                ae_f32x2_loadip(a00, px0, 4*sizeof(ae_f32x2));
                ae_f32x2_loadip(a01, px1, 4*sizeof(ae_f32x2));
                ...
                a10 = AE_SEL32_HH(b11, b10);
                a11 = AE_SEL32_LL(b11, b10);
                ae_f32x2_storex(a01, py0, 2*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a00, py0, 3*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a11, py1, 5*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a10, py1, 4*2*sizeof(ae_f32x2));
            }
            {
                ae_f32x2_loadip(a00, px0, 4*sizeof(ae_f32x2));
                ae_f32x2_loadip(a01, px1, 4*sizeof(ae_f32x2));
                ...
                a10 = AE_SEL32_HH(b11, b10);
                a11 = AE_SEL32_LL(b11, b10);
                ae_f32x2_storex(a01, py0, 6*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a00, py0, 7*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a11, py1, 1*2*sizeof(ae_f32x2));
                ae_f32x2_storex(a10, py1, 0*2*sizeof(ae_f32x2));
            }
        }
