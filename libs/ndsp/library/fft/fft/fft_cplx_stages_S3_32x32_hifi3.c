...
{                                                     \
    ...
    s0 = AE_SUB32S(x0, s0);                           \
    x0 = s1;                                          \
    d0=AE_MULFP32X2RAS(AE_SEL32_LH(d0,d0),0x6ED9EBA1);\
    x1=AE_ADDSUB32S(s0,d0);                           \
    x2=AE_SUBADD32S(s0,d0);                           \
}       
#endif
/*
DFT5 algorithm:
...
{                                         \
  ...
  y0 = AE_ADD32S(x0, AE_ADD32S(s1, s2));  \
  tmp=(x0);                               \
  AE_MULAFP32X2RAS(tmp,s1,0x278DDE6E);    \
  AE_MULAFP32X2RAS(tmp,s2,0x98722192);    \
  y1=(tmp);                               \
  y4 = y1;                                \
  tmp=(x0);                               \
  AE_MULAFP32X2RAS(tmp,s1,0x98722192);    \
  AE_MULAFP32X2RAS(tmp,s2,0x278DDE6E);    \
  y2=(tmp);                               \
  y3 = y2;                                \
  d1i = AE_SEL32_LH(d1, d1);              \
  d2i = AE_SEL32_LH(d2, d2);              \
  tmp=  AE_MULFP32X2RAS(d1i,0x8643C7B3);  \
  AE_MULAFP32X2RAS(tmp,d2i,0xB4C373EE);   \
  t0 = AE_SUBADD32S(0,tmp );              \
  tmp = AE_MULFP32X2RAS(d1i,0xB4C373EE);  \
  AE_MULSFP32X2RAS(tmp,d2i,0x8643C7B3);   \
  t1=AE_SUBADD32S(0, tmp);                \
  x1 = AE_ADD32S(y1, t0);                 \
  x4 = AE_SUB32S(y4, t0);                 \
  x2 = AE_ADD32S(y2, t1);                 \
  x3 = AE_SUB32S(y3, t1);                 \
...
int fft_stageS3_DFT2_first_32x32(const int32_t *tw, const int32_t *x, int32_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  /* hifi3z: 15 cycles per stage, unroll=4*/
...
int fft_stageS3_DFT2_last_32x32(const int32_t *tw, const int32_t *x, int32_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  /* hifi3z: 3 cycles per stage, unroll=1*/
  __Pragma("loop_count min=6");
  for (j = 0; j < stride; j++)
  {
    ae_int32x2 x0, x1, y0, y1;
...
int fft_stageS3_DFT3_first_32x32(const int32_t *tw, const int32_t *x, int32_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  const int stride = ((int32_t)AE_MOVAD32_L(AE_MULF16SS_00(AE_MOVDA16(N),AE_MOVDA16(10923))))>>16;
  NASSERT_ALIGN8(x);
  NASSERT_ALIGN8(y);
  int i;
...
int fft_stageS3_DFT4_last_32x32(const int32_t *tw, const int32_t *x, int32_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  /* hifi3z: 7 cycles per stage, unroll=1*/
  px0 = (ae_int32x2 *)(0 * sizeof(ae_int32x2) + (uintptr_t)x);
  py0 = (ae_int32x2 *)(0 * sizeof(ae_int32x2) + (uintptr_t)y);
  __Pragma("loop_count min=3");
  for (j = 0; j < stride; j++)
...
{ 
...

  {
    ...
    {
      int  flag = 0;
     // uint32_t  dflag = (_v > 1) ? 0x80000000 >> (30 - NSA(_v) - 1) :
     //   0x00000000;
      const int tw_inc0 = sizeof(complex_fract32);
      ...
      px0 = (ae_int32x2 *)(x);
      ptwd = (const ae_int32x2 *)tw; 
      /* hifi3z: 15 cycles per stage, unroll=1*/
      flag = _v;
      __Pragma("loop_count min=3");
      for (i = 0; i< stride; i++)
      {
      ...
      //  if (flag==_v) {flag=0;}
        XT_MOVEQZ(py_inc, (1)* sizeof(complex_fract32), flag);
        XT_MOVEQZ(tw_inc, (3 * (tw_step - 1)*sizeof(complex_fract32) + sizeof(complex_fract32)), flag);
        XT_MOVEQZ(flag, _v, flag);

        AE_L32X2_XP(tw1, ptwd, tw_inc0);
        AE_L32X2_XP(tw2, ptwd, tw_inc0);
...
int fft_stageS3_DFT5_first_32x32(const int32_t *tw, const int32_t *x, int32_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  ae_int32x2 * restrict py0;
  const ae_int32x2 * restrict ptwd;
  const int stride = ((int32_t)AE_MOVAD32_L(AE_MULF16SS_00(AE_MOVDA16(N),AE_MOVDA16(6554))))>>16;
  NASSERT_ALIGN8(x);
  NASSERT_ALIGN8(y);
  int i;
  ...
  ptwd = (const ae_int32x2 *)tw;
  shift = 3;
  /* hifi3z: 26 cycles per stage, unroll=1*/
...
int fft_stageS3_DFT5_last_32x32(const int32_t *tw, const int32_t *x, int32_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  shift = 3;
  /* hifi3z: 20 cycles per stage, unroll=1*/
  __Pragma("loop_count min=12");
  for (j = 0; j < _v; j++)
  {
    ae_int32x2 x0, x1, x2, x3, x4;
...
{ 
  ...
  NASSERT_ALIGN8(y);
  {
    ...
    for (j = 0; j < _v; j++)
    {
      ...
      px0 = (ae_int32x2 *)(j * sizeof(ae_int32x2) + (uintptr_t)x);
      py0 = (ae_int32x2 *)(j * sizeof(ae_int32x2) + (uintptr_t)y);
      /* hifi3z: 27 cycles per stage, unroll=1*/
      __Pragma("loop_count min=1");
      for (i = 0; i < (stride/ _v); i++)
      {
        AE_L32X2_IP(tw1, ptwd, sizeof(ae_int32x2));
    ...
    #else
        {
          ...
          px0 = (ae_int32x2 *)(x);
          ptwd = (const ae_int32x2 *)tw;
          /* hifi3z: 34 cycles per stage, unroll=1*/
          flag = _v;
          __Pragma("loop_count min=1");
          for (i = 0; i< stride; i++)
          {
...
int ifft_stageS3_DFT4_first_32x32(const int32_t *tw, const int32_t *x, int32_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  */
  ...
  /* hifi3z: 10 cycles per pipeline stage in steady state with unroll=1*/
...
ALIGN(16) static const int32_t __fft8_tw1[] =
{
