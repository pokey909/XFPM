...
/*
    ...
    C code optimized for Fusion/HiFi3z
*/
/*===========================================================================
...
ALIGN(16) static const int16_t  ifft_twd_r8[12]  = {
...
#define SWAP_REIM(_x) { _x = AE_SHORTSWAP(_x); _x = AE_SEL16_5432(_x, _x);}
#if 1
static void stage_first_DFT4xI2_inpl(int16_t *x, int16_t *twd, int N)
{
    ae_int32x2 * restrict px32x2 = (ae_int32x2 *)x;
    ...
    /*fusion: 11 cycles per pipeline stage in steady state with unroll=1 */
    /* hifi3z: 7 cycles per pipeline stage in steady state with unroll=1*/
    for (i = 0; i < (N >> 3); i++)
    {
        ae_int32x2 r0, r1, r2, r3;
        r1 = AE_L32X2_X(px32x2, stride);
        r3 = AE_L32X2_X(px32x2, stride * 3);
        r2 = AE_L32X2_X(px32x2, stride * 2);
        AE_L32X2_XP(r0, px32x2, 8);

        x0 = AE_MOVINT16X4_FROMF32X2(r0); 
        x1 = AE_MOVINT16X4_FROMF32X2(r1);
        x3 = AE_MOVINT16X4_FROMF32X2(r3);
        x2 = AE_MOVINT16X4_FROMF32X2(r2);
...
static void stage_last_DFT4xI2_inpl(int16_t *x, int16_t *y, int N)
{
    ...
    ae_int32x2 * restrict py10 = (ae_int32x2 *)(y);
    ae_int32x2 * restrict py11 = (ae_int32x2 *)(y + N);
    ae_int32x2 * restrict py12 = (ae_int32x2 *)(y + N / 2);
    ae_int32x2 * restrict py13 = (ae_int32x2 *)(y + 3 * N / 2);
    ...
    for (i = 0; i<(N / 4); i += 2)
    {
        ...
        /*Fusion  10 cycles per pipeline stage in steady state with unroll=1 */
        ...
        AE_S32X2_X(AE_MOVINT32X2_FROMINT16X4(h4), py10, ibitrev);
        AE_S32X2_X(AE_MOVINT32X2_FROMINT16X4(h5), py11, ibitrev);
        AE_S32X2_X(AE_MOVINT32X2_FROMINT16X4(h2), py12, ibitrev);
        AE_S32X2_X(AE_MOVINT32X2_FROMINT16X4(h3), py13, ibitrev);
...
static void stage_inner_DFT4xI2_inpl_merged(int16_t *x, int16_t *y, const int16_t *twd, int N, int stride)
{
    ...
    for (i = 0; i < N / 8; i++)
    {
        /*Fusion:  13 cycles per pipeline stage in steady state with unroll=1 */
...
static void stage_last_DFT2xI2_inpl(int16_t *x, int16_t *y, int N)
{
    ...
    for (i = 0; i<(N / 4); i += 2)
    {
        /*Fusion:  9 cycles per pipeline stage in steady state with unroll=1 */
...
static int stage_inner_DFT4xI2_inpl_ref_ds(int16_t *x, int16_t *y, const int16_t *twd, int N, int stride)
{
    ...
    {   /* ~ 20 cycles */
        ...
        t21 = AE_SLAI32(t21, 8);
        t31 = AE_SLAI32(t31, 8);
        /* todo needed same twiddles for Hifi3 and Hifi3z:
        - Hifi3z  used single table for both ifft_cplx16x16 and fft_cplx16x16;
        - Hifi3   used different tables.
        */
        _CONJ32(t10);
        _CONJ32(t20);
        _CONJ32(t30);
