...
static int stage_last_iDFT4_16x16_ie(const int16_t *tw, const int16_t *x, int16_t *y, int N, int *v, int tw_step, int *bexp)
{
    ...
    ae_int32x2 * restrict _py0;
    ae_int16x4 * restrict _px0;
    ...
    {   
      /* 7 cycles per pipeline stage in steady state with unroll=1 */
        ...
        AE_L16X4_XP(_x0, _px0, stride*sizeof(complex_fract16));
        AE_L16X4_XP(_x1, _px0, stride*sizeof(complex_fract16));
        AE_L16X4_XP(_x2, _px0, stride*sizeof(complex_fract16));
        AE_L16X4_XP(_x3, _px0, sizeof(_x3)-3 * stride*sizeof(complex_fract16));
        // DFT4
        {
            ae_int16x4 t0, t1, t2, t3; 
            AE_ADDANDSUBRNG16RAS_S2(_x0, _x2);
            AE_ADDANDSUBRNG16RAS_S2(_x1, _x3);
                      
            _x3 = AE_MOVINT16X4_FROMF16X4(AE_MUL16JS(AE_MOVF16X4_FROMINT16X4(_x3))); 
            
            t0 = AE_ADD16S(_x0, _x1);
            t1 = AE_SUB16S(_x0, _x1);
            t2 = AE_ADD16S(_x2, _x3);
            t3 = AE_SUB16S(_x2, _x3);
            _x0 = t0; _x2 = t1; _x3 = t2; _x1 = t3;
        }

        AE_S32X2_XP(AE_MOVINT32X2_FROMINT16X4(_x0), _py0, stride*sizeof(complex_fract16));
        AE_S32X2_XP(AE_MOVINT32X2_FROMINT16X4(_x1), _py0, stride*sizeof(complex_fract16));
        AE_S32X2_XP(AE_MOVINT32X2_FROMINT16X4(_x2), _py0, stride*sizeof(complex_fract16));
        AE_S32X2_XP(AE_MOVINT32X2_FROMINT16X4(_x3), _py0, sizeof(_x3)-3 * stride*sizeof(complex_fract16));
    }
    return shift;
} //stage_last_iDFT4_16x16_ie, hifi3z
...
static int stage_last_iDFT4_16x16_ie(const int16_t *tw, const int16_t *x, int16_t *y, int N, int *v, int tw_step, int *bexp)
{
    ...
    for (i = 0; i < N / 8; i++)
    {
        ...
        AE_S32X2_XP(AE_MOVINT32X2_FROMINT16X4(_x3), _py, sizeof(_x3)-3 * stride*sizeof(complex_fract16));
    }
    return shift;
} //stage_last_iDFT4_16x16_ie, hifi3z
...
static int stage_first_iDFT4_16x16_ie(const int16_t *tw, const int16_t *x, int16_t *y, int N, int *v, int tw_step, int *bexp)
{
    ...
    for (i = 0; i < N / R / 2; i++)
    {
        ...
        AE_L32X2_XP(tmp3, _px, sizeof(_x3)-3 * stride*sizeof(complex_fract16));
...
static int stage_first_iDFT4_16x16_ie(const int16_t *tw, const int16_t *x, int16_t *y, int N, int *v, int tw_step, int *bexp)
{
    ...
    *v *= R;
    return shift;
} //stage_first_iDFT4_16x16_ie, hifi3
#endif //#if  (XCHAL_HAVE_HIFI3Z)
