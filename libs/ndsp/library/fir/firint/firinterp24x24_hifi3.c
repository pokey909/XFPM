...
typedef struct tag_firinterp24x24_t
{
...
} firinterp24x24_t, *firinterp24x24_ptr_t;
...
size_t firinterp24x24_alloc( int D, int M )
{
  ...
  sz= ( ALIGNED_SIZE( sizeof( firinterp24x24_t ), 4 )
           + // Delay line
...
firinterp24x24_handle_t firinterp24x24_init( void * objmem, 
                                             ...
                                             const f24 * restrict h )
{
  firinterp24x24_ptr_t firinterp;
  void *               ptr;
  f24 *                delayLine;
  int                  delayLen;
  f24 *                coefBuf;
  ...
  ptr       = objmem;
  firinterp = (firinterp24x24_ptr_t)(ALIGNED_ADDR( ptr, 4 ));
  ptr       = firinterp + 1;
...
{
  firinterp24x24_ptr_t firinterp = (firinterp24x24_ptr_t)_firinterp;
...
static f24 * firinterp2_proc( f24 * restrict y,
                              f24 *          cbegin,
                              f24 *          cend,
                              f24 * restrict cpos,
                        const f24 * restrict x,
                        ...
                        int D, int N, int M )
{
  ...
  for ( n=0; n<(N>>3); n++ )
  {
    ...
    //
    // Inner loop kernel: process 4 taps for 8 accumulators. Actually we perform
    // M+4 MACs for each accumulator. 4 extra MACs fall on zero coefficients
    // appended to the impulse response to avoid a 1-sample response delay.
    //
    ...
    AE_S24RA64S_IP( q0, Y,  +2*4 ); //  0
    AE_S24RA64S_IP( q1, Y,  +2*4 ); //  2
    AE_S24RA64S_IP( q2, Y,  +2*4 ); //  4
    AE_S24RA64S_IP( q3, Y,  +2*4 ); //  6
    AE_S24RA64S_IP( q4, Y,  +2*4 ); //  8
    AE_S24RA64S_IP( q5, Y,  +2*4 ); // 10
    AE_S24RA64S_IP( q6, Y,  +2*4 ); // 12
    AE_S24RA64S_XP( q7, Y, -13*4 ); // 14
    ...
    // Q23 <- Q16.47 - 24 w/ rounding and saturation
    AE_S24RA64S_IP( q0, Y, +2*4 ); //  1
    AE_S24RA64S_IP( q1, Y, +2*4 ); //  3
    AE_S24RA64S_IP( q2, Y, +2*4 ); //  5
    AE_S24RA64S_IP( q3, Y, +2*4 ); //  7
    AE_S24RA64S_IP( q4, Y, +2*4 ); //  9
    AE_S24RA64S_IP( q5, Y, +2*4 ); // 11
    AE_S24RA64S_IP( q6, Y, +2*4 ); // 13
    AE_S24RA64S_IP( q7, Y, +1*4 ); // 15
  }
...
static f24 * firinterp3_proc( f24 * restrict y,
                              f24 *          cbegin,
                              f24 *          cend,
                              f24 * restrict cpos,
                        const f24 * restrict x,
                        ...
                        int D, int N, int M )
{
  ...
  for ( n=0; n<(N>>3); n++ )
  {
    ...
    // Q23 <- Q16.47 - 24 w/ rounding and saturation
    AE_S24RA64S_IP( q0, Y,  +3*4 ); //  0
    AE_S24RA64S_IP( q1, Y,  +3*4 ); //  3
    AE_S24RA64S_IP( q2, Y,  +3*4 ); //  6
    AE_S24RA64S_IP( q3, Y,  +3*4 ); //  9
    AE_S24RA64S_IP( q4, Y,  +3*4 ); // 12
    AE_S24RA64S_IP( q5, Y,  +3*4 ); // 15
    AE_S24RA64S_IP( q6, Y,  +3*4 ); // 18
    AE_S24RA64S_XP( q7, Y, -20*4 ); // 21
    ...
    // Q23 <- Q16.47 - 24 w/ rounding and saturation
    AE_S24RA64S_IP( q0, Y,  +3*4 ); //  1
    AE_S24RA64S_IP( q1, Y,  +3*4 ); //  4
    AE_S24RA64S_IP( q2, Y,  +3*4 ); //  7
    AE_S24RA64S_IP( q3, Y,  +3*4 ); // 10
    AE_S24RA64S_IP( q4, Y,  +3*4 ); // 13
    AE_S24RA64S_IP( q5, Y,  +3*4 ); // 16
    AE_S24RA64S_IP( q6, Y,  +3*4 ); // 19
    AE_S24RA64S_XP( q7, Y, -20*4 ); // 22
    ...
    // Q23 <- Q16.47 - 24 w/ rounding and saturation
    AE_S24RA64S_IP( q0, Y, +3*4 ); //  2
    AE_S24RA64S_IP( q1, Y, +3*4 ); //  5
    AE_S24RA64S_IP( q2, Y, +3*4 ); //  8
    AE_S24RA64S_IP( q3, Y, +3*4 ); // 11
    AE_S24RA64S_IP( q4, Y, +3*4 ); // 14
    AE_S24RA64S_IP( q5, Y, +3*4 ); // 17
    AE_S24RA64S_IP( q6, Y, +3*4 ); // 20
    AE_S24RA64S_IP( q7, Y, +1*4 ); // 23
  }
...
static f24 * firinterp4_proc( f24 * restrict y,
                              f24 *          cbegin,
                              f24 *          cend,
                              f24 * restrict cpos,
                        const f24 * restrict x,
                        ...
                        int D, int N, int M )
{
  ...
  for ( n=0; n<(N>>3); n++ )
  {
    ...
    // Q23 <- Q16.47 - 24 w/ rounding and saturation
    AE_S24RA64S_IP( q0, Y,  +4*4 ); //  0
    AE_S24RA64S_IP( q1, Y,  +4*4 ); //  4
    AE_S24RA64S_IP( q2, Y,  +4*4 ); //  8
    AE_S24RA64S_IP( q3, Y,  +4*4 ); // 12
    AE_S24RA64S_IP( q4, Y,  +4*4 ); // 16
    AE_S24RA64S_IP( q5, Y,  +4*4 ); // 20
    AE_S24RA64S_IP( q6, Y,  +4*4 ); // 24
    AE_S24RA64S_XP( q7, Y, -27*4 ); // 28
    ...
    // Q23 <- Q16.47 - 24 w/ rounding and saturation
    AE_S24RA64S_IP( q0, Y,  +4*4 ); //  1
    AE_S24RA64S_IP( q1, Y,  +4*4 ); //  5
    AE_S24RA64S_IP( q2, Y,  +4*4 ); //  9
    AE_S24RA64S_IP( q3, Y,  +4*4 ); // 13
    AE_S24RA64S_IP( q4, Y,  +4*4 ); // 17
    AE_S24RA64S_IP( q5, Y,  +4*4 ); // 21
    AE_S24RA64S_IP( q6, Y,  +4*4 ); // 25
    AE_S24RA64S_XP( q7, Y, -27*4 ); // 29
    ...
    // Q23 <- Q16.47 - 24 w/ rounding and saturation
    AE_S24RA64S_IP( q0, Y,  +4*4 ); //  2
    AE_S24RA64S_IP( q1, Y,  +4*4 ); //  6
    AE_S24RA64S_IP( q2, Y,  +4*4 ); // 10
    AE_S24RA64S_IP( q3, Y,  +4*4 ); // 14
    AE_S24RA64S_IP( q4, Y,  +4*4 ); // 18
    AE_S24RA64S_IP( q5, Y,  +4*4 ); // 22
    AE_S24RA64S_IP( q6, Y,  +4*4 ); // 26
    AE_S24RA64S_XP( q7, Y, -27*4 ); // 30
    ...
    // Q23 <- Q16.47 - 24 w/ rounding and saturation
    AE_S24RA64S_IP( q0, Y, +4*4 ); //  3
    AE_S24RA64S_IP( q1, Y, +4*4 ); //  7
    AE_S24RA64S_IP( q2, Y, +4*4 ); // 11
    AE_S24RA64S_IP( q3, Y, +4*4 ); // 15
    AE_S24RA64S_IP( q4, Y, +4*4 ); // 19
    AE_S24RA64S_IP( q5, Y, +4*4 ); // 23
    AE_S24RA64S_IP( q6, Y, +4*4 ); // 27
    AE_S24RA64S_IP( q7, Y, +1*4 ); // 31
  }
...
static f24 * firinterpX_proc( f24 * restrict y,
                              f24 *          cbegin,
                              f24 *          cend,
                              f24 * restrict cpos,
                        const f24 * restrict x,
                        ...
                        int D, int N, int M )
{
  ...
  for ( n=0; n<(N>>3); n++ )
  {
    ...
    __Pragma("loop_count min=1")
      ...
      // Q23 <- Q16.47 - 24 w/ rounding and saturation
      AE_S24RA64S_XP( q0, Y, D*4 );       // d+0*D
      AE_S24RA64S_XP( q1, Y, D*4 );       // d+1*D
      AE_S24RA64S_XP( q2, Y, D*4 );       // d+2*D
      AE_S24RA64S_XP( q3, Y, D*4 );       // d+3*D
      AE_S24RA64S_XP( q4, Y, D*4 );       // d+4*D
      AE_S24RA64S_XP( q5, Y, D*4 );       // d+5*D
      AE_S24RA64S_XP( q6, Y, D*4 );       // d+6*D
      AE_S24RA64S_XP( q7, Y, (1-7*D)*4 ); // d+7*D
    }

    Y = (ae_f24*)( (uintptr_t)Y + 7*D*4 );
  }
