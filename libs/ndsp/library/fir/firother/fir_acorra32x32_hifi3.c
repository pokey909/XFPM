                      ...
                      const int32_t  * restrict x, int N )
{
    ...
    ae_f32x2 x0, x1, x2, y0, y0_low, y0_high, d0;
    ae_f64 a0, a1, a2, a3;
    ae_valign ar, ay;
    ae_int32x2 t0, t1, t2;
...
#pragma concurrent
    ...
    for (n = 0; n < (N&~3); n += 4)
    {
        ...
        for (m = 0; m < (M >> 1) - 1; m++){
            ...
            //AE_LA32X2_IP(t0, ay, pY); 
            //y0 = (t0);
            AE_L32_IP(t0, castxcc(ae_int32, pY), +4);
            y0_high = t0;
            AE_L32_IP(t0, castxcc(ae_int32, pY), +4);
            y0_low = t0;

            AE_MULAF32R_HH(a0, x0, y0_high);
            AE_MULAF32R_LL(a0, x0, y0_low);

            AE_MULAF32R_LL(a1, x0, y0_high);
            AE_MULAF32R_LH(a1, y0_low, x1);

            AE_MULAF32R_HH(a2, x1, y0_high);
            AE_MULAF32R_LL(a2, x1, y0_low);

            AE_MULAF32R_LL(a3, x1, y0_high);
            AE_MULAF32R_LH(a3, y0_low, x2);
        }
    ...
    if (N)
    {
        ...
        AE_S32_L_IP(AE_SEL32_LH(x0,x0), castxcc(ae_int32, pR), 4);
        if (N>1) AE_S32_L_IP(x0, castxcc(ae_int32, pR), 4);
        x0 = AE_ROUND32X2F48SASYM(a2, a2);
        if (N>2) AE_S32_L_IP(AE_SEL32_LH(x0,x0), castxcc(ae_int32, pR), 4);
    }
} /* fir_acorra32x32() */
