                   ...
                   const int16_t * restrict  x, int N )
{
    ...
    for (n=0; n<(N&~3); n+=4)
    {
        ...
        X12 = (AE_SEL32_LH(XN0, XN0));
        X34 = (AE_SEL32_LH(XN1, XN1));
        X01 = AE_SEL32_HL(XN0, t);
        X23 = AE_SEL32_HL(XN1, XN0);

        AE_L32_IP(t, castxcc(ae_int32, pH), sizeof(int32_t)); 
        H01 = AE_MOVF16X4_FROMINT32X2(t);
        q0 = q1 = AE_ZERO();
        ...
        AE_S32_L_XC(AE_SEL32_LH(XN0, XN0), castxcc(ae_int32, p), 4);
        AE_S32_L_XC(XN0, castxcc(ae_int32, p), 4);
        AE_S32_L_XC(AE_SEL32_LH(XN1, XN1), castxcc(ae_int32, p), 4);
        AE_S32_L_XC(XN1, castxcc(ae_int32, p), 4);
    ...
    if(N)
    {
        ...
        XX = AE_ROUND16X4F32SASYM(t0, t0);
        AE_S16_0_IP(XX,castxcc(ae_int16, pY), sizeof(int16_t));
        AE_S32_L_XC(AE_SEL32_LH(XN0, XN0), castxcc(ae_int32, p), 4);

        if (N>1) 
        { 
          t0 = AE_TRUNCA32X2F64S(q1, q1, 16);
          XX = AE_ROUND16X4F32SASYM(t0, t0); 
          AE_S16_0_IP(XX,  castxcc(ae_int16, pY), sizeof(int16_t)); 
          AE_S32_L_XC(XN0, castxcc(ae_int32, p), 4); 
        }
                   ...
                   const int16_t * restrict  x, int N )
{
    ...
    ae_valign D_va0, D_va2;
    ae_valign ay;
    ae_valign x_align;

    ae_int16x4 d01, d23, d12;
    ae_int16x4 d0, d1;
