...
/*  Element-wise minimum of two vectors
	...
	Restrictions: x, y, z should not overlap
 * 		        : Inputs should be 8 byte aligned
*/
#include "common_fpu.h"

#if !HAVE_VFPU && !HAVE_FPU
DISCARD_FUN(void, vec_eleminf, (float32_t* restrict z, float32_t* restrict x, float32_t* restrict y, int N))
#elif HAVE_VFPU

#define __HiFi3_3z_NDSP__ 1
#if __HiFi3_3z_NDSP__
void vec_eleminf (float32_t* restrict z, const float32_t* restrict x, const float32_t* restrict y, int N)
{
...
#elif __REFC_OOB__
#include "math.h"
void vec_eleminf (float32_t* restrict z, float32_t* restrict x, float32_t* restrict y, int N)
{
	int i;
