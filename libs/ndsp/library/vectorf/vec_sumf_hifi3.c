 ...
 */
/*  Vector sum: Sum of entire Input vector
...
#define  __OPT__ 0 //Optimized version might not be bitexact wiith unoptimized version

#if !HAVE_VFPU && !HAVE_FPU
DISCARD_FUN(float32_t, vec_sumf, (  const float32_t * restrict x, int N ))
#elif HAVE_VFPU

#if __HiFi3_3z_NDSP__
	#if __OPT__
float32_t vec_sumf ( const float32_t * restrict x, int N )
{
	xtfloatx2* px;
	xtfloatx2 xtt=0.0f, ztt=0.0f;
	xtfloat y=0, xt;
	int i;
